...
export const getApprovedCoursesU = async (
  courseDbRepository: ReturnType<CourseDbRepositoryInterface>
): Promise<(CourseInterface & { thumbnailUrl?: string })[]> => {
  const courses = await courseDbRepository.getApprovedCourses();

  if (!courses || courses.length === 0) {
    throw new AppError('No approved courses found', HttpStatusCodes.NOT_FOUND);
  }
