...
export const getRejectedCoursesByInstructorU = async (
  instructorId: string,
  ...
  const courses = await courseDbRepository.getRejectedCoursesByInstructor(instructorId);

  if (!courses || courses.length === 0) {
    throw new AppError('No rejected courses found for this instructor', HttpStatusCodes.NOT_FOUND);
  }
