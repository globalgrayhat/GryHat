...
export const getApprovedCoursesByInstructorU = async (
  instructorId: string,
  courseDbRepository: ReturnType<CourseDbRepositoryInterface>
): Promise<(CourseInterface & { thumbnailUrl?: string })[]> => {
  if (!instructorId) {
    throw new AppError('Instructor ID is required', HttpStatusCodes.BAD_REQUEST);
  }

  const courses = await courseDbRepository.getApprovedCoursesByInstructor(instructorId);

  if (!courses || courses.length === 0) {
    throw new AppError('No approved courses found for this instructor', HttpStatusCodes.NOT_FOUND);
  }
