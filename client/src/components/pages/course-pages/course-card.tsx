...
type CourseCardData = CourseInterface & {
  instructorId?: any;            // may be object with firstName/lastName/profilePic
  category?: any;                // may be object with name
...
const CourseCard: React.FC<Props> = (props) => {
  const {
    title, description, duration, level, price, isPaid, rating,
    thumbnailUrl, coursesEnrolled,
  } = props;

  // Normalize category name
  const categoryRaw = (props as any)?.category;
  const categoryName = typeof categoryRaw === "string" ? categoryRaw : categoryRaw?.name;

  // Normalize instructor data
  const instructor = (props as any)?.instructorId;
  ...
  // Cover URL
  const coverUrl = getFullUrl(thumbnailUrl || (props as any)?.thumbnail?.url || "");

  // Price / FREE
  const paidFlag = typeof isPaid === "boolean" ? isPaid : (typeof price === "number" ? price > 0 : false);
  const isFree = !paidFlag || (typeof price === "number" && price === 0);

  // Enrollment
  const enrolled =
    typeof (props as any)?.enrolledCount === "number"
      ? (props as any)?.enrolledCount
        ...
        <div className="relative aspect-[16/9] w-full overflow-hidden">
          ...
          <div className="absolute top-2 left-2 flex flex-wrap gap-2">
            ...
            {(typeof price === "number" || typeof isPaid === "boolean") && (
              ...
              >
                {isFree ? "FREE" : formatToINR?.(price as number) ?? `₹${price}`}
              </span>
            )}
          </div>
