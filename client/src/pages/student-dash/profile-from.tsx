...
const ProfileForm: React.FC<Props> = ({ editMode, setEditMode }) => {
  ...
  const handleSubmit = async (profileInfo: UpdateProfileInfo & { profilePic?: File | null }) => {
    try {
      ...
      setUpdated(!updated);
      toast.error(error?.data?.message || (error?.message ?? 'Error'), { position: toast.POSITION.BOTTOM_RIGHT });
    }
  };
      ...
      <form onSubmit={formik.handleSubmit} className='space-y-6'>
        ...
        <div className='flex items-center gap-4'>
          <div className='relative'>
            ...
            <div className='absolute right-0 bottom-0 bg-white dark:bg-gray-700 rounded-full p-1 shadow -translate-x-1 translate-y-1'>
              <label htmlFor='file_input' className='cursor-pointer flex items-center justify-center w-8 h-8'>
                <PhotoIcon className='w-5 h-5' />
              </label>
              <input id='file_input' type='file' accept='image/*' onChange={handleFileChange} className='hidden' />
            </div>
          </div>
          ...
          <div className='flex-1'>
            <div className='flex items-center justify-between'>
              <div>
                <div className='text-lg font-semibold text-gray-800 dark:text-gray-100'>{formik.values.firstName} {formik.values.lastName}</div>
                <div className='text-sm text-gray-500 dark:text-gray-400'>{formik.values.qualification}</div>
              </div>

              <div className='flex items-center gap-2'>
                {!editMode ? (
                  <button type='button' onClick={() => setEditMode(true)} className='flex items-center gap-2 px-3 py-2 rounded-md border hover:bg-gray-50 dark:hover:bg-gray-700'>
                    <PencilSquareIcon className='w-4 h-4' />
                    <span className='text-sm'>{t('settings.edit') || 'Edit'}</span>
                  </button>
                ) : (
                  <div className='flex items-center gap-2'>
                    <button type='button' onClick={() => { setEditMode(false); formik.resetForm(); }} className='flex items-center gap-2 px-3 py-2 rounded-md border hover:bg-gray-50 dark:hover:bg-gray-700'>
                      <XMarkIcon className='w-4 h-4' />
                      <span className='text-sm'>{t('settings.cancel') || 'Cancel'}</span>
                    </button>
                    <button type='submit' className='flex items-center gap-2 px-3 py-2 rounded-md bg-blue-600 text-white hover:bg-blue-700'>
                      <CheckIcon className='w-4 h-4' />
                      <span className='text-sm'>{t('settings.save') || 'Save'}</span>
                    </button>
                  </div>
                )}

                <button type='button' className='flex items-center gap-1 px-2 py-2 rounded-md border hover:bg-gray-50 dark:hover:bg-gray-700' title='Settings'>
        ...
        <div className='grid grid-cols-1 md:grid-cols-2 gap-4'>
          ...
          <div className='md:col-span-2'>
            <label htmlFor='about' className='block text-xs text-gray-500 mb-1'>{t('settings.about') || 'About'}</label>
            <textarea
              id='about'
              name='about'
        ...
        {editMode && (
          <div className='flex justify-end gap-3'>
            <button type='button' onClick={() => { setEditMode(false); formik.resetForm(); }} className='flex items-center gap-2 px-4 py-2 rounded-md border'>
              <XMarkIcon className='w-4 h-4' />
              <span>{t('settings.cancel') || 'Cancel'}</span>
            </button>
            <button type='submit' className='flex items-center gap-2 px-4 py-2 rounded-md bg-blue-600 text-white'>
              <CheckIcon className='w-4 h-4' />
...
export const InputWithIcon: React.FC<InputWithIconProps> = ({ id, name, value, onChange, disabled = false, placeholder = '', Icon, className = '' }) => {
  return (
    <div className={`relative ${className}`}>
      ...
      <input
        ...
        className={`pl-12 pr-3 py-2 w-full text-sm bg-transparent border border-gray-200 dark:border-gray-700 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 ${disabled ? 'opacity-80' : ''}`}
      />
    </div>
  );
};